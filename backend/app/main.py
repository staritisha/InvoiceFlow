...
from app.schemas import (
    ...
    ReminderResponse, RecurringBillingCreate, RecurringBillingResponse,
    InvoiceItemCreate, InvoiceItemResponse, AIFollowupResponse
)
...
app = FastAPI(title="InvoiceFlow API", lifespan=lifespan)
...
def monthly_revenue(db: Session = Depends(get_db)):
    ...
    return [
        {"month": month_names[int(r.month) - 1], "amount": float(r.amount)}
        for r in results
    ]  
    results = (
        db.query(
...
def export_invoices_csv(db: Session = Depends(get_db)):
    ...
    with open(file_path, mode="w", newline="") as file:
        ...
        for invoice in invoices:
            writer.writerow([invoice.id, invoice.invoice_number, invoice.customer_id, invoice.status, invoice.total_amount, invoice.due_date, invoice.notes])
...
def run_recurring_billing_scheduler(db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"message": "Recurring billing scheduler completed", "created_invoices": created_invoices, "count": len(created_invoices)}
